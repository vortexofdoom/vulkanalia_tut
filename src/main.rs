...
unsafe fn create_pipeline(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    // Once pipeline creation is finished, we no longer need the shader modules
...
unsafe fn create_framebuffers(device: &Device, data: &mut AppData) -> Result<()> {
    data.framebuffers = data
        ...
        .iter()
        .map(|v| {
            let attachments = &[data.color_image_view, data.depth_image_view, *v];
...
unsafe fn create_texture_image(
    ...
    let img = File::open("./resources/viking_room.png")?;
...
unsafe fn transition_image_layout(
    ...
    // let aspect_mask = if new_layout == vk::ImageLayout::DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL {
    //     match format {
        ...
        for index in &model.mesh.indices {
            ...
            let vertex = Vertex {
                ...
                tex_coord: glm::vec2(
                    ...
                    // correcting for 0 at bottom vs top of image
